            ...
            <div className="max-w-screen-sm lg:max-w-screen-md xl:max-w-screen-xl mx-auto py-16 ">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-8">
                    {subscriptions.map((subscription) => (
                        <div key={subscription.id} className="rounded-tl-3xl rounded-br-3xl border-2 bg-white pt-8 mx-6 md:mx-0 hover:bg-oliveGreen border-oliveGreen hover:text-lightWhite hover:shadow-2xl hover:shadow-oliveGreen transition-all duration-500">
                            <h4 className="text-center text-2xl py-6 uppercase font-bold">{subscription.pack}</h4>
                            <h1 className="text-center text-3xl font-extrabold ">${subscription.price}</h1>
                            <h2 className="text-center font-semibold text-base ">/Monthly</h2>
                            <ul className="text-base pt-6 px-12">
                                <div className="flex py-2"><FaCheck className="font-extrabold pt-1 text-xl" /><p className="pl-2">Borrow maximum {subscription.borrow_limit} books at a time</p></div>
                                <div className="flex py-2"><FaCheck className="font-extrabold pt-1 text-2xl" /><p className="pl-2">Borrow each books for maximum {subscription.return_limit} days</p></div>
                                <div className="flex py-2"><FaCheck className="font-extrabold pt-1 text-xl" /><p className="pl-2">Borrowed book limit {subscription.book_limit} per month</p></div>
                            </ul>
